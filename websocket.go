...
func Accept(w http.ResponseWriter, r *http.Request, opts Options) (*Websocket, error) {
	...
	return New(conn, connBuf, clientKey, ServerMode, opts), nil
}
...
func New(conn io.Closer, connBuf *bufio.ReadWriter, clientKey ClientKey, mode Mode, opts Options) *Websocket {
	setDefaults(&opts)
...
func (ws *Websocket) ReadMessage(ctx context.Context) (*Message, error) {
	...
	for {
		...
		frame, err := ReadFrame(ws.connBuf, ws.mode, ws.maxFrameSize)
		if err != nil {
		...
		case OpcodePing:
			...
			if err := WriteFrame(ws.connBuf, ws.mask(), frame); err != nil {
				return nil, err
			}
			continue
		case OpcodePong:
...
func (ws *Websocket) closeWithError(code StatusCode, err error) error {
	...
	if err := WriteFrame(ws.connBuf, ws.mask(), NewCloseFrame(code, reason)); err != nil {
