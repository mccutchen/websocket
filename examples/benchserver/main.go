...
func newDebugHooks(ctx context.Context, logger *slog.Logger) websocket.Hooks {
	...
	return websocket.Hooks{
		OnCloseHandshakeStart: func(key websocket.ClientKey, initiatedBy string, code websocket.StatusCode, err error) {
			logger.Log(ctx, levelForErr(err), "OnCloseHandshakeStart", "client", key, "initiatedBy", initiatedBy, "code", code, "err", err)
		},
		OnCloseHandshakeDone: func(key websocket.ClientKey, initiatedBy string, code websocket.StatusCode, err error) {
			logger.Log(ctx, levelForErr(err), "OnCloseHandshakeDone", "client", key, "initiatedBy", initiatedBy, "code", code, "err", err)
		},
